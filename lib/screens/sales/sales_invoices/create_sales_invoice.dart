...
class CreateSalesInvoice extends StatefulWidget {
  const CreateSalesInvoice({Key? key}) : super(key: key);

  @override
  _CreateSalesInvoiceState createState() => _CreateSalesInvoiceState();
}

class _CreateSalesInvoiceState extends State<CreateSalesInvoice> {
  var notesSwap = false;
                    ...
                    child: Row(
                      children: [
                        ...
                        if(!Responsive.isMobile(context))
                          Text("Create Sales Invoice",style: Theme.of(context).textTheme.headline6!.copyWith(color: txtColor),),
                        if(!Responsive.isMobile(context))
...
class _AdditionalChargeTextFieldsState extends State<AdditionalChargeTextFields> {
  ...
  Widget build(BuildContext context) {
    WidgetsBinding.instance!.addPostFrameCallback((timeStamp) {
      _nameController.text = _CreateSalesInvoiceState.additionalChargeList[widget.index]
          ?? '';
    });
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        SizedBox(
          ...
          child: TextFormField(
            ...
            onChanged: (v) => _CreateSalesInvoiceState.additionalChargeList[widget.index] = v,
            decoration: new InputDecoration(
                contentPadding: EdgeInsets.all(defaultPadding/2),
                focusedBorder: OutlineInputBorder(
                  borderSide: BorderSide(color: primaryColor, width: 1.0),
