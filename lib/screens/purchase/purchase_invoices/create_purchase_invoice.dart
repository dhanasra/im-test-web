...
class CreatePurchaseInvoice extends StatefulWidget {
  const CreatePurchaseInvoice({Key? key}) : super(key: key);

  @override
  _CreatePurchaseInvoiceState createState() => _CreatePurchaseInvoiceState();
}

class _CreatePurchaseInvoiceState extends State<CreatePurchaseInvoice> {
  var notesSwap = false;
                    ...
                    child: Row(
                      children: [
                        ...
                        if(!Responsive.isMobile(context))
                          Text("Create Purchase Invoice",style: Theme.of(context).textTheme.headline6!.copyWith(color: txtColor),),
                        if(!Responsive.isMobile(context))
...
class _AdditionalChargeTextFieldsState extends State<AdditionalChargeTextFields> {
  ...
  Widget build(BuildContext context) {
    WidgetsBinding.instance!.addPostFrameCallback((timeStamp) {
      _nameController.text = _CreatePurchaseInvoiceState.additionalChargeList[widget.index]
          ?? '';
    });
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        SizedBox(
          ...
          child: TextFormField(
            ...
            onChanged: (v) => _CreatePurchaseInvoiceState.additionalChargeList[widget.index] = v,
            decoration: new InputDecoration(
                contentPadding: EdgeInsets.all(defaultPadding/2),
                focusedBorder: OutlineInputBorder(
                  borderSide: BorderSide(color: primaryColor, width: 1.0),
